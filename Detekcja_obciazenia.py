...
CAMERA_INDEXES = [0, 1]          # dwie kamery
...
global_weights = {idx: 0.0 for idx in CAMERA_INDEXES}
latest_frames = {idx: None for idx in CAMERA_INDEXES}
...
def render_total_image(total_with_mode: float, mode: str, w=520, h=260):
...
def camera_worker(camera_index=0, label="Kamera"):
    ...
    if cap is None:
        ...
        # ustaw placeholder, żeby podgląd pokazywał, że kamera nie działa
        with global_lock:
...
# ===============================
# GUI: SUMA + START/STOP + TRYB + PODGLĄD (ZAWSZE 2 OKNA)
# ===============================
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        ...
        ttk.Label(controls, text="Tryb:").grid(row=2, column=0, sticky="w", pady=(10, 0))
        row1 = ttk.Frame(controls)
        row1.grid(row=3, column=0, sticky="w")
    ...
    def _update_preview_windows(self):
        # Jeśli preview OFF -> zamknij okna
        if not self.preview.get():
            if self.preview_windows_open:
                self._close_preview_windows()
            return

        # Preview ON -> ZAWSZE pokazuj 2 okna
        with global_lock:
    ...
    def update_ui(self):
        with global_lock:
            total_all = sum(global_weights.values())

        mode = self.mode.get()
        total_with_mode = total_all + MODE_VALUES.get(mode, 0)

        img = render_total_image(total_with_mode, mode)
        ...
        # odśwież podgląd (jeśli włączony)
